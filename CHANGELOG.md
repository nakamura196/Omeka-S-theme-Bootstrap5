...
Rebuilt on the Omeka S 4.2.1 core templates, for Omeka S 4.1 and later.
Merges this repository with [ldasjp8/Omeka-S-theme-Bootstrap5](https://github.com/ldasjp8/Omeka-S-theme-Bootstrap5) 4.0.x;
the setting names follow 4.0.x.

### Added
- Light / dark mode switch in the header (light, dark, same as the device).
- Main and accent color settings.
- Item and media pages: breadcrumbs, copy URL, citation, share (X, Facebook, email),
  IIIF manifest link with the IIIF logo, JSON-LD link.
- Page size selector on browse pages.
- Open Graph and Twitter card tags; sticky header on wide screens; print styles.

### Changed
- Bootstrap CSS 5.3.8 and Font Awesome Free 6 are bundled instead of loaded from a CDN.
  Bootstrap's JavaScript is no longer used.
- `hero_image`, `hero_button_url` and `hero_button_text` (2.x) are read only as fallbacks for
  `top_image`, `top_url` and the button label.

### Removed
- Hero carousel, faceted search sidebar, `list_display_properties` and `thumbnail_display_mode`.
